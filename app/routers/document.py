...
def list_documents(company_id: UUID, db: Session = Depends(get_db)):
    return get_documents_by_company(db, company_id)


@router.post("/{company_id}/documents", response_model=DocumentBase)
def upload_document(
    company_id: UUID,
...
@router.put("/{company_id}/documents/{document_id}", response_model=DocumentBase)
